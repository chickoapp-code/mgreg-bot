"""Хендлеры, реализующие сценарий регистрации Тайного гостя."""
...
async def confirm_registration(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    client: PlanfixClient = callback.bot.get("planfix_client")
    try:
    ...
    if existing:
        await state.update_data(existing_contact=existing)
    ...
    await _create_contact(callback, state, contact_data)
...
async def duplicate_update(callback: CallbackQuery, state: FSMContext) -> None:
    await callback.answer()
    data = await state.get_data()
    contact_data = build_contact_data(data)

    await _create_contact(callback, state, contact_data, update_existing=True)
...
async def _create_contact(
    ...
    *,
    ...
    client: PlanfixClient = callback.bot.get("planfix_client")
    try:
        response = await client.ensure_contact(contact_data, update_existing=update_existing)
    except PlanfixError as exc:
    ...
    if contact_id:
        ...
        admin_chat_id = callback.bot.get("admin_chat_id")
        admin_name = callback.bot.get("admin_name")
        planfix_base_url = callback.bot.get("planfix_base_url")
