...
**📌 Способы запуска:**
- **Системная служба (systemd)** - рекомендуемый способ для продакшена (Шаг 5)
- **Без службы** - простые способы запуска (см. раздел "Альтернативные способы запуска")
...
# Используется для создания и проверки подписи sig в URL вида:
# /webapp/start?taskId=123&guestId=456&form=resto_a&sig=abc123&ts=1234567890
# Генерируется: python3 -c "import secrets; print(secrets.token_urlsafe(32))"
WEBAPP_HMAC_SECRET=сгенерируйте_случайную_строку_для_подписи_webapp

# YFORMS_WEBHOOK_SECRET - секрет для проверки подписи вебхуков от Яндекс Форм
# Используется для верификации заголовка X-Forms-Signature в запросах от форм
# Этот же секрет должен быть указан в настройках вебхука каждой Яндекс Формы
# Генерируется: python3 -c "import secrets; print(secrets.token_urlsafe(32))"
...
Для генерации безопасных секретов используйте один из способов:
...
# Генерация YFORMS_WEBHOOK_SECRET (выполните команду еще раз для получения другого значения)
python3 -c "import secrets; print(secrets.token_urlsafe(32))"
```
...
# Или base64 (44 символа)
openssl rand -base64 32
```

**Способ 3: Онлайн генератор**

Вы можете использовать любой онлайн генератор случайных строк, например:
- https://www.random.org/strings/
- Выберите длину: 32-64 символа
...
**Важно:**
- Каждый секрет должен быть уникальным (разные значения для WEBAPP_HMAC_SECRET и YFORMS_WEBHOOK_SECRET)
- Секреты должны быть достаточно длинными (минимум 32 символа)
- Храните секреты в безопасности - не публикуйте их и не коммитьте в git
...
## Альтернативные способы запуска (без systemd)

Если вы не хотите использовать systemd службу, можно запустить сервис другими способами:
...
⚠️ **Недостаток:** Процесс остановится при закрытии терминала или разрыве SSH соединения.

---

### Способ 2: Запуск в фоне с nohup
...
# Запомните PID процесса (он будет выведен)
echo $! > bot.pid
```
...
# Внутри screen запустите сервис
python -m bot.main

# Отключение от screen: нажмите Ctrl+A, затем D
...
### Способ 4: Запуск через tmux

**Установка tmux (если еще не установлен):**
```bash
...
# Внутри tmux запустите сервис
python -m bot.main

# Отключение от tmux: нажмите Ctrl+B, затем D
# (сессия продолжит работать в фоне)
```
...
### Способ 5: Запуск через Supervisor (альтернатива systemd)
...
### Обновление кода (для запуска без systemd)

**Если используете nohup:**
```bash
cd ~/mgreg-bot
kill $(cat bot.pid)  # Остановка
source .venv/bin/activate
git pull
pip install -r requirements.txt
nohup python -m bot.main > bot.log 2>&1 &
...
screen -r crmbot  # или tmux attach -t crmbot
